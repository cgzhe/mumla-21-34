...
Mumla is a fork and continuation of [Plumble](https://github.com/acomminos/Plumble), a robust GPLv3 Mumble client for Android originally written by Andrew Comminos.

This repository is based on the original Mumla project ([https://github.com/quite/mumla](https://github.com/quite/mumla)) and **preserves the complete Git commit history**, with custom modifications applied on top.

## About This Fork

This fork includes the following modifications and optimizations:

- **Reduced SDK Requirements**: Supports only Android 5.0 to Android 15.0 (API 21-34). Support for higher Android versions has been removed.
- **Certificate Fixes**: Resolved certificate usage issues present in the original version.
- **Small Screen Adaptation**: Optimized display for small screen devices.
- **Non-Touchscreen Support**: Adapted for Android devices without touchscreen (such as TV boxes, car head units, etc.).

## Relationship with the Original Project

- **Original Project**: [https://github.com/quite/mumla](https://github.com/quite/mumla)
- **This Repository**: Forked from the original, with complete commit history preserved
- **Key Differences**: As listed above, focusing on compatibility with older Android versions and specific use cases

## Original Project Maintenance Status

> The following is from the original Mumla project README:

Current maintainer [Daniel Lublin](https://lublin.se) ([@quite@mstdn.social](https://mstdn.social/@quite)) has very little time to do voluntary work on Mumla. The focus is strictly on maintaining stability and security. This includes migrations to newer Android SDKs, as they become requirements by Google/Alphabet for even getting updates published on Google Play.
...
If you want to help out translating Mumla, the project is [on Weblate](https://hosted.weblate.org/engage/mumla/) -- thanks for gratis hosting of our libre project!
...
The Android SDK need to be specified as usual, for example by setting `ANDROID_SDK_ROOT`, or writing it to local.properties as `echo >local.properties sdk.dir=/home/user/Android/Sdk`
...
We currently use Android Gradle Plugin (AGP) version 8.x, which should come bundled with NDK 25.1.8937393 that we currently use. It is typically installed in a directory in `~/Android/Sdk/ndk/`. Using newer NDK might give build errors.
...
I'm also maintaining a container image that is used to build Mumla in [.gitlab-ci.yml](.gitlab-ci.yml). Source at https://gitlab.com/quite/android-sdk-ndk
